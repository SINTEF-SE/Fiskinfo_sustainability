...
Printout: At the top of ``àpi_requests.py``, a parameter called printout is is used to tell if debug parameters should be printed out. Set to false if this is not neccessary, it will speed up the program. The debug output mainly shows the API calls and the parameters used in the calls.
