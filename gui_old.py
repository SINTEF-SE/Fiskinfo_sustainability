...
class MainWindow(QMainWindow):
    ...
    def getGear_button_clicked(self):
        ...
        client = DatafangstClient()
        client.get(ep.gear, info_log = self.infoOutput.isChecked(), csv_file=toCsvFile, append_csv=self.appendCsv.isChecked())

    def getGearGroups_button_clicked(self):
        toCsvFile = "output/gearGroups.csv" if self.storeCsv.isChecked() else ""
        client = DatafangstClient()
        client.get(ep.gear_groups, info_log = self.infoOutput.isChecked(), csv_file=toCsvFile, append_csv=self.appendCsv.isChecked())

    def getGearMainGroups_button_clicked(self):
        toCsvFile = "output/gearMainGroups.csv" if self.storeCsv.isChecked() else ""
        client = DatafangstClient()
        client.get(ep.gear_main_groups, info_log = self.infoOutput.isChecked(), csv_file=toCsvFile, append_csv=self.appendCsv.isChecked())
    
    def getVessels_button_clicked(self):
        toCsvFile = "output/vessels.csv" if self.storeCsv.isChecked() else ""
        client = DatafangstClient()
        client.get(ep.vessels, info_log = self.infoOutput.isChecked(), csv_file=toCsvFile, append_csv=self.appendCsv.isChecked())
        
    def getVesselsFuel_button_clicked(self):
        toCsvFile = "output/vesselsFuel.csv" if self.storeCsv.isChecked() else ""
        client = DatafangstClient()
        client.get(ep.fuel, sDate = self.startDateEdit.date(), eDate = self.stopDateEdit.date(), auth = True, info_log = self.infoOutput.isChecked(), csv_file=toCsvFile, append_csv=self.appendCsv.isChecked())
    ...
    def getVesselsLiveFuel_button_clicked(self):
        ...
        client = DatafangstClient()
        client.get(ep.live_fuel, auth = True, info_log = self.infoOutput.isChecked(), csv_file=toCsvFile, append_csv=self.appendCsv.isChecked())
        
    def getVesselsBenchmarks_button_clicked(self):
        toCsvFile = "output/benchmarks.csv" if self.storeCsv.isChecked() else ""
        client = DatafangstClient()
        client.get(ep.vessel_benchmarks, auth = True, info_log = self.infoOutput.isChecked(), csv_file=toCsvFile, append_csv=self.appendCsv.isChecked())
        
    def getUser_button_clicked(self):
        toCsvFile = "output/user.csv" if self.storeCsv.isChecked() else ""
        client = DatafangstClient()
        client.get(ep.user, auth = True, info_log = self.infoOutput.isChecked(), csv_file=toCsvFile, append_csv=self.appendCsv.isChecked())
       
    def getTrips_button_clicked(self):
        toCsvFile = "output/trips.csv" if self.storeCsv.isChecked() else ""
        vesselId = self.fiskdirId if self.myVessel.isChecked() else ""
    ...
    def getHaul_button_clicked(self):
        ...
        client.get(ep.haul,
                    ...
                    vesselGroups=self.vesselCombo.checked_items_data(),
                    speciesGroups=self.speciesCombo.checked_items_data(),
                    locationsGroups = splitCatchLocation(self.locationText.toPlainText()),
                    vesselId = vesselId,
                    info_log=self.infoOutput.isChecked(),
                    csv_file=toCsvFile,
                    append_csv=self.appendCsv.isChecked())

    def getPrice_button_clicked(self):  #Needs checking, not workiong!!!!!!!!!!!!
        priceUrl = 'https://data.ssb.no/api/pxwebapi/v2/tables/09654/data?lang=no&valueCodes[PetroleumProd]=035&valueCodes[ContentsCode]=Priser&valueCodes[Tid]='
        toCsvFile = "output/price.csv" if self.storeCsv.isChecked() else ""

        client = DatafangstClient(priceUrl)
        client.get(ep.price,
                    sDate = self.startDateEdit.date(),
                    eDate = self.stopDateEdit.date(),
                    info_log=self.infoOutput.isChecked(),
    ...
    def bank_button_clicked(self):
        ...
        kpi01_pngFile = "output/kpi01"          # Do not specify extension .png on these files !!
        ...
        kpi_05(gui_data)
         ...
         # Find total number of vessels in in group if input data has changed
        ...
        #r.createPlot(gui_data, 'KPI-01: EEOI [g CO2 /(fangst*nm)]', kpi01_pngFile)
        #r.createPlot(gui_data, 'KPI-02: FUI [g CO2 /fangst]', kpi02_pngFile)
        #r.createPlot(gui_data, 'KPI-05:', kpi05_pngFile)
        ...
        if (toCsvFile != ""):
            ...
            r.json_to_csv(jsonData, toCsvFile)
    ...
    def kpi01_button_clicked(self):
        ...
        toPngFile = "output/kpi01"                   # Do not specify extension .png on these files !!"
        ...
        # create plot
        # r.createPlot(gui_data, 'KPI-01: EEOI [g CO2 /(fangst*nm)]', fName = toPngFile, show = True)
        ...
        if (toCsvFile != ""):
            ...
            #ep.json_to_pandas_csv(jsonArray[0], toCsvFile)
    ...
    def kpi05_button_clicked(self):
        ...
        # create plot
        #r.createPlot(gui_data, 'KPI-05:', show = True)
